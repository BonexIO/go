...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X github.com/BonexIO/go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X github.com/BonexIO/go/support/app.version=%s", version)
